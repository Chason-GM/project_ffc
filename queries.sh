...
echo "$($PSQL "select name from teams inner join games on teams.team_id = games.winner_id where year = 2018 limit 1")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "select distinct name from teams inner join games on teams.team_id = games.winner_id or teams.team_id = games.opponent_id where year = 2014 and round = 'Eighth-Final'")"
